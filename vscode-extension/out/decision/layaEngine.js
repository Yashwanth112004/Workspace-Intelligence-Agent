...
exports.LayaDecisionEngine = void 0;
...
class LayaDecisionEngine {
    ...
    decide(query) {
        ...
        for (const opName of rawOps) {
            if ((0, operationRegistry_1.isValidOperation)(opName)) {
                const opParams = {};
                if (opName === 'index.search' || opName === 'component.explain') {
                    if (targetParams.query)
                        opParams.query = targetParams.query;
                    if (targetParams.file_path)
                        opParams.file_path = targetParams.file_path;
                }
                else if (opName === 'impact.analyze') {
                    if (targetParams.symbol)
                        opParams.symbol = targetParams.symbol;
                    else if (targetParams.file_path)
                        opParams.file_path = targetParams.file_path;
                }
                else if (opName === 'dependency.check') {
                    if (targetParams.query)
                        opParams.scope = targetParams.query;
                }
