...
export class WiaAgentPanel {
    ...
    public static readonly viewType = 'wia.agentPanel';
    ...
    private activityHistory: Array<{ timestamp: string; request: string; operations: string[]; duration_ms: number; requiresLLM: boolean }> = [];

    public static createOrShow(
        extensionUri: vscode.Uri,
        apiClient: WiaApiClient,
        secretStorage: WiaSecretStorage,
        layaEngine: LayaDecisionEngine,
        executor: WiaExecutor,
        envRepair: EnvironmentRepairEngine,
        ...
        WiaAgentPanel.currentPanel = new WiaAgentPanel(
            panel,
            extensionUri,
            apiClient,
            secretStorage,
    ...
    public setWorkspace(workspaceRoot: string, repoId: string | null) {
        this.currentRootPath = workspaceRoot;
        this.currentRepoId = repoId;
    ...
    private async handleSaveAiConfig(provider: string, model: string, apiKey?: string) {
    ...
    private async handleTestConnection(provider: string, apiKey?: string) {
    ...
    private async handleNaturalLanguageQuery(query: string) {
        if (!this.currentRootPath || !query.trim()) return;
        ...
        try {
            const askResult = await this.executor.executeOperation(
                { name: 'component.explain', parameters: { query } },
