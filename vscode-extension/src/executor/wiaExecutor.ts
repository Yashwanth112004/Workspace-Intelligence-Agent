...
import { WIAOperationCall, WIAOperationResult, getOperationDefinition, isValidOperation } from '../registry/operationRegistry';
import { WiaApiClient } from '../apiClient';

export class WiaExecutor {
    private terminal: vscode.Terminal | null = null;

    constructor(private readonly apiClient: WiaApiClient) {}

    /**
     * Detect actual runnable project commands from repository manifests (package.json, pyproject.toml, Makefile, Cargo.toml).
     */
    public detectProjectCommands(workspaceRoot: string): { run?: string; build?: string; test?: string; lint?: string } {
        const detected: { run?: string; build?: string; test?: string; lint?: string } = {};
        ...
        if (fs.existsSync(pkgPath)) {
            try {
                ...
                if (scripts.dev) detected.run = 'npm run dev';
                else if (scripts.start) detected.run = 'npm start';
                else if (scripts.serve) detected.run = 'npm run serve';

                if (scripts.build) detected.build = 'npm run build';
                if (scripts.test) detected.test = 'npm test';
                if (scripts.lint) detected.lint = 'npm run lint';
            } catch (e) {}
        }
        ...
        if (fs.existsSync(path.join(workspaceRoot, 'Cargo.toml'))) {
            if (!detected.run) detected.run = 'cargo run';
            if (!detected.build) detected.build = 'cargo build';
            if (!detected.test) detected.test = 'cargo test';
        }

        // 4. Go detection
        if (fs.existsSync(path.join(workspaceRoot, 'go.mod'))) {
            if (!detected.run) detected.run = 'go run .';
            if (!detected.build) detected.build = 'go build';
            if (!detected.test) detected.test = 'go test ./...';
        }
     ...
     */
    public executeInTerminal(command: string, name: string = 'WIA Project Command'): void {
     ...
     */
    public async executeOperation(call: WIAOperationCall, workspaceRoot: string): Promise<WIAOperationResult> {
        const startTime = Date.now();

        if (!isValidOperation(call.name)) {
            return {
                operation: call.name,
                status: 'failed',
        ...
        const def = getOperationDefinition(call.name)!;
        ...
        if (def.category === 'project') {
            ...
            if (call.name === 'project.run') cmdToRun = commands.run;
            else if (call.name === 'project.build') cmdToRun = commands.build;
            else if (call.name === 'project.test') cmdToRun = commands.test;
            else if (call.name === 'project.lint') cmdToRun = commands.lint;
    ...
    private mapOperationToCliArgs(call: WIAOperationCall, _workspaceRoot: string): string[] {
    ...
    private runWiaCli(args: string[], cwd: string): Promise<{ stdout: string; stderr: string; exitCode: number; parsedData?: any }> {
