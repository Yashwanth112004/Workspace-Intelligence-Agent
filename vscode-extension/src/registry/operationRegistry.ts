...
export interface WIAOperationDefinition {
    name: string;
    category: 'workspace' | 'repository' | 'index' | 'dependency' | 'environment' | 'project' | 'architecture' | 'component' | 'impact';
    description: string;
...
export interface WIAOperationCall {
    name: string;
    parameters?: Record<string, any>;
}

export interface WIAOperationResult {
    operation: string;
...
export const WIA_OPERATION_REGISTRY: Record<string, WIAOperationDefinition> = {
...
export function isValidOperation(opName: string): boolean {
    return Object.prototype.hasOwnProperty.call(WIA_OPERATION_REGISTRY, opName);
}

export function getOperationDefinition(opName: string): WIAOperationDefinition | undefined {
    return WIA_OPERATION_REGISTRY[opName];
}
