...
class NvidiaNimProvider(AIProvider):
    ...
    def __init__(
        ...
        self.api_key = api_key or os.environ.get("NVIDIA_API_KEY") or os.environ.get("NIM_API_KEY") or ""
        self.model = model or os.environ.get("NVIDIA_MODEL") or self.DEFAULT_MODEL
        self.endpoint = endpoint or os.environ.get("NVIDIA_ENDPOINT") or self.DEFAULT_ENDPOINT
    ...
    def generate(self, prompt: str, context: str, options: dict[str, Any] | None = None) -> str:
        ...
        try:
            ...
            if err.code == 401:
                return f"Authentication Failed (401): The provided NVIDIA API key is invalid or expired. Check your NVIDIA_API_KEY."
            elif err.code == 429:
...
class OpenAIProvider(AIProvider):
    """OpenAI API provider extension point."""
    ...
    def generate(self, prompt: str, context: str, options: dict[str, Any] | None = None) -> str:
        ...
        return "OpenAI provider integration configured."
...
class AIProviderFactory:
    ...
    def get_provider(
        ...
        p_name = (provider_name or os.environ.get("WIA_AI_PROVIDER") or "").lower().strip()

        if p_name in ("nvidia", "nvidia_nim", "nim") or (not p_name and os.environ.get("NVIDIA_API_KEY")):
