...
class HierarchicalSummarizerEngine:
    ...
    def generate_all_summaries(repo: Repository, nodes: List[FileNode], symbols: List[ASTSymbol]) -> List[WorkspaceSummary]:
        ...
        # Step 1 & 2: Function & File Summaries
        ...
        for f in files:
            ...
            fn_summary_texts = []
            for fn in fn_symbols[:5]: # Max 5 per file for brevity
                fn_sum = f"Function {fn.name}({', '.join(fn.parameters)}): " + (fn.docstring or "Performs operational logic within file.")
            ...
            # File Summary (combining function summaries and file structure)
            file_prompt = f"""Summarize the purpose of file '{f.relative_path}' ({f.language or 'Code'}, {f.loc_count} LOC).
Symbols/Functions defined: {[s.name for s in f_symbols]}
Function details: {'; '.join(fn_summary_texts[:3])}

Provide a concise 2-3 sentence overview of this file's responsibility and exports."""
            
            file_sum_text = LLMClient.generate_completion(file_prompt, system_prompt="You are a code understanding assistant.")
        ...
        # Step 3 & 4: Folder Summaries (Child Folder -> Parent Folder bottom-up)
        folder_summary_map: Dict[str, str] = {}
        
        for d in dirs:
            dir_path = d.relative_path
            
            # Gather immediate file summaries inside this directory
            ...
            # Gather subfolder summaries inside this directory
            ...
            if combined_context:
                folder_prompt = f"""Summarize the directory '{dir_path}' based on its contents:
Contents context:
{chr(10).join(combined_context[:8])}

Provide a clear 2-3 sentence description of what this directory component provides."""
        ...
        repo_prompt = f"""Generate a high-level architecture overview for the repository '{repo.name}'.
...
Provide a comprehensive architectural summary highlighting:
1. Primary purpose of the workspace
2. Core modules & technology stack
3. Key components and data flow overview"""
